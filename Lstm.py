...
def backward(activations, clipping=True):
    ...
    for t in reversed(range(input_length)):
        ...
        dcnext = fs[t] * dc             # für Ableitung von L nach h[t-1] -> nur die ersten  (hidden_size) Einträge von dz nehmen!


        # backward through the embedding
        dwe = dz[-emb_size:, :]     # für Ableitung von L nach w[t] -> nur die letzten (emb_size) Einträge von dz nehmen!
